"""
Payments MCP service (accounts/payments split — payments domain).

Exposes the PAYMENTS-domain tool over the Model Context Protocol (streamable HTTP
transport): make_payment. It holds **no authorization policy of its own** — that
lives at the payments gateway. This MCP server sits behind the payments gateway
(PEP #1) and the make_payment tool calls the Payments REST API **through the
payments gateway (PEP #2)**, forwarding the caller's delegated identity. Ping
Authorize (via the AuthZEN adapter) is consulted at both hops; the payments-api
then executes the money move against the accounts domain (a further,
independently-authorized hop).

`BANK_API_BASE_URL` points at the payments gateway's route to the payments-api
(e.g. http://kong-payments:8000/payments).
"""
...
mcp = FastMCP(
    ...
    instructions=(
        "Retail banking PAYMENTS tools for an AI assistant acting on behalf of a "
        "bank customer. Use make_payment to move money between accounts. Every "
        "action is authorized by the bank's policy gateway; the tool may return "
        "\"authorized\": false with a POLICY DENIED or STEP-UP REQUIRED message, "
        "which you must relay to the customer without retrying."
    ),
    host=HOST,
    port=PORT,
    stateless_http=True,
...
def _incoming_token(ctx: Context | None, principal: str) -> str:
    """The delegated token to forward to the Payments API through the gateway."""
    try:
...
def _incoming_user_token(ctx: Context | None) -> str | None:
    ...
    PEP can check her consented scopes (e.g. banking:payments:transfer)."""
    try:
        req = ctx.request_context.request  # type: ignore[union-attr]
...
# ---------------------------------------------------------------------------
# COAZ (OpenID AuthZEN MCP profile) declarations — payments domain only.
# See the accounts-mcp for the shared rationale; the make_payment mapping carries
# the amount/currency/description in the context so the step-up policy can gate on
# amount, and the from/to accounts on the resource for the RAR creditor match.
# ---------------------------------------------------------------------------
...
if __name__ == "__main__":
    logger.info("Starting Payments MCP server on %s:%s (Payments API via %s)",
